...
public class TextComunicator implements IComunicator, InputProcessor {
    ...
    public void newText(String fullText, float width, float height, float textSize, boolean cursorOn, boolean big){
        // initialize font
        if(big) {
            try {
        ...
        newText(fullText,width,height, textSize, cursorOn);
    }

    @Deprecated
    public void newText(String fullText, float width, float height, float textSize, boolean cursorOn){
    ...
    public void draw(){
        batch.begin();
        if(text != null){
        	//if(height > 200 && ( height - 200 - ((font.getCapHeight()*text.length())/(Gdx.graphics.getWidth() - 300))*font.getCapHeight() < 0 )) height += font.getCapHeight();
        	font.draw(batch,text,width,height,textSize,-5,true);
        }
        batch.end();
    }
